...
describe("CloudAccountRefreshService", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(CloudAccountRepo.getAccount).mockImplementation(
  ...
  it("persists the first strike across service reset and blocks on the second operation", async () => {
    ...
    await expect(
      CloudAccountRefreshService.refreshAccessToken(request),
    ).rejects.toBeInstanceOf(OAuthTokenRefreshError);
  ...
  it("serializes transitions from initial strike to terminal block", async () => {
    vi.mocked(GoogleAPIService.refreshAccessToken).mockRejectedValue(
      invalidGrantError(),
    );
  ...
  it("clears failure state for a blocked account", async () => {
    persistedAccount.health = {
      validation: {
        status: "requires_action",
        reason: "VALIDATION_REQUIRED",
    ...
    expect(persistedAccount.health).toEqual({
      validation: persistedAccount.health?.validation,
    });
  });
});
