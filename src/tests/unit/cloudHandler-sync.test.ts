...
vi.mock(
  "@/modules/cloud-account/services/GoogleAPIService",
  async (importOriginal) => {
    const original =
      await importOriginal<
        typeof import("@/modules/cloud-account/services/GoogleAPIService")
      >();
    return {
      ...original,
      GoogleAPIService: {
...
vi.mock(
  "@/modules/cloud-account/persistence/antigravityCredentialStore",
  async (importOriginal) => {
    const actual =
      await importOriginal<
        typeof import("@/modules/cloud-account/persistence/antigravityCredentialStore")
      >();
    return {
      ...actual,
...
describe("IdeAccountImportAdapter.syncFromIde", () => {
        ...
        from: () => ({
          where: (condition: { __key?: string }) => ({
            all: () => {
              ...
              const value =
                mockDataByDbPath[activeMockDbPath]?.[key] ?? mockData[key];
              if (value === undefined) {
                return [];
              }
              return [{ value }];
  ...
  it("should sync from agy CLI token file without reading IDE SQLite paths", async () => {
    vi.spyOn(fs, "existsSync").mockReturnValue(true);
    ...
    const { GoogleAPIService } =
      await import("@/modules/cloud-account/services/GoogleAPIService");
    vi.mocked(GoogleAPIService.getUserInfo).mockResolvedValue(
      createMockUserInfo("cli@example.com", "CLI User"),
    );
    vi.spyOn(CloudAccountRepo, "getAccounts").mockResolvedValue([]);
    vi.spyOn(CloudAccountRepo, "addAccount").mockResolvedValue();

    const account = await IdeAccountImportAdapter.syncFromIde("agy");
  ...
  it("should prefer unified oauth token when present", async () => {
    ...
    const oldB64 = Buffer.from(
      ProtobufUtils.createOAuthTokenInfo(
        "access-old",
        "refresh-old",
        1700000000,
      ),
  ...
  it("reads enterprise project preference from IDE unified state when syncing new account", async () => {
    ...
    const projectPayload = ProtobufUtils.createStringValuePayload(
    ...
    mockData["antigravityUnifiedStateSync.oauthToken"] = unifiedB64;
    mockData["antigravityUnifiedStateSync.enterprisePreferences"] =
      enterprisePreferenceB64;
  ...
  it("preserves existing token metadata and proxy settings when syncing existing account", async () => {
    ...
    const addAccountSpy = vi
      .spyOn(CloudAccountRepo, "addAccount")
      .mockResolvedValue();
  ...
  it("should refresh the IDE token when user info rejects the stored access token", async () => {
    ...
    vi.mocked(GoogleAPIService.getUserInfo)
      ...
      .mockResolvedValueOnce(
        createMockUserInfo("refreshed@example.com", "Refreshed User"),
      );
    ...
    expect(GoogleAPIService.getUserInfo).toHaveBeenNthCalledWith(
      1,
      staleAccessToken,
    );
    expect(GoogleAPIService.getUserInfo).toHaveBeenNthCalledWith(
  ...
  it("preserves existing unified topic state when injecting a new OAuth token", async () => {
    ...
    const writtenValue = (
      unifiedWrite?.args[0] as { value?: string } | undefined
    )?.value;
    ...
    expect(
      ProtobufUtils.extractOAuthTokenInfoFromUnifiedState(writtenTopic),
    ).toEqual({
  ...
  it("clears legacy IDE OAuth state when injecting new-format unified state", async () => {
    ...
    mockData["antigravityUnifiedStateSync.oauthToken"] =
      ProtobufUtils.createUnifiedOAuthToken(
        "old-access",
        "old-refresh",
        1699999999,
      );
  ...
  it("treats the first confirmed invalid_grant as retryable during manual quota refresh and switch messaging", async () => {
    ...
    const statusSpy = vi
      .spyOn(RepoWithMock, "setAccountStatus")
      .mockResolvedValue();
    const { OAuthTokenRefreshError: RefreshError } =
      await import("@/modules/cloud-account/services/GoogleAPIService");
    const refreshError = new RefreshError(
      "invalid_grant",
      400,
      "default",
      "expired or revoked",
    ...
    const { CloudAccountRefreshService: RefreshServiceWithMock } =
      await import("@/modules/cloud-account/services/CloudAccountRefreshService");
    vi.spyOn(RefreshServiceWithMock, "refreshAccessToken").mockRejectedValue(
      refreshError,
    );
    const { formatSwitchRefreshError, refreshAccountQuota } =
      await import("@/modules/cloud-account/ipc/handler");

    await expect(refreshAccountQuota("first-strike-id")).rejects.toBe(
      refreshError,
    );
  ...
  it("should keep pre-2.0 product versions out of the credential store", async () => {
    ...
    expect(
      AdapterWithMock.shouldInjectTokenIntoCredentialStore("classic"),
    ).toBe(false);
  });
  ...
  it("should always route agy CLI token injection to credential store", async () => {
    ...
    expect(AdapterWithMock.shouldInjectTokenIntoCredentialStore("agy")).toBe(
      true,
    );
  });
  ...
  it("should route Classic Antigravity 2.0+ token injection to credential store", () => {
    ...
    const injectCloudTokenSpy = vi.spyOn(
      CredentialStoreInjectionAdapter,
      "injectCloudToken",
    );
    ...
    expect(shouldInjectTokenIntoCredentialStoreSpy).toHaveBeenCalledWith(
      undefined,
    );
...
describe("syncLocalAccount ORPC error mapping", () => {
  ...
  it("maps unsupported agy CLI sync to a bad request error", () => {
    const error = toSyncLocalAccountORPCError(
      new Error(AGY_SYNC_FROM_IDE_UNSUPPORTED_MESSAGE),
    );
...
describe("cloud switch fail-fast path", () => {
  ...
  it("should fail fast without rollback or forced restart when inject fails", async () => {
    ...
    vi.doMock(
      "@/modules/cloud-account/persistence/credential-store-injection-adapter",
      () => ({
        CredentialStoreInjectionAdapter: {
    ...
    vi.doMock("@/modules/cloud-account/services/GoogleAPIService", () => ({
      GoogleAPIService: {
        ...
        normalizeRefreshedOAuthClientKey: vi.fn(
          (_currentToken: unknown, refreshedClientKey?: string) =>
            refreshedClientKey,
        ),
      },
    }));
    ...
    const { switchCloudAccount } =
      await import("@/modules/cloud-account/ipc/handler");
    await expect(switchCloudAccount("acc-1")).rejects.toThrow(
      "Switch failed: inject_failed",
    );
...
describe("cloud oauth client key backfill", () => {
  ...
  it("marks Classic active from target state when credential store mode has no SQLite account email", async () => {
    ...
    vi.doMock("@/modules/antigravity-runtime/switch/switchGuard", () => ({
      runWithSwitchGuard: async (_owner: string, fn: () => Promise<void>) =>
        fn(),
    }));
    ...
    expect(
      listedAccounts.find((account) => account.id === "acc-1")
        ?.is_active_classic,
    ).toBe(false);
    expect(
      listedAccounts.find((account) => account.id === "acc-2")
        ?.is_active_classic,
    ).toBe(true);
  });
  ...
  it("keeps Classic credential-store active target when SQLite still reports previous email", async () => {
    ...
    vi.doMock(
      "@/modules/account/persistence/antigravity-state-database",
      () => ({
