...
describe("Forge package ignore policy", () => {
  it("keeps runtime roots required by the packaged app", () => {
    ...
    expect(
      shouldIgnorePackagePath("/.vite/renderer/main_window/assets/index.js"),
    ).toBe(false);
    expect(
      shouldIgnorePackagePath("/node_modules/better-sqlite3/package.json"),
    ).toBe(false);
  ...
  it("prunes renderer and build-time packages from packaged node_modules", () => {
    ...
    expect(
      shouldIgnorePackagePath(
        "/node_modules/@icons-pack/react-simple-icons/index.mjs",
      ),
    ).toBe(true);
    expect(shouldIgnorePackagePath("/node_modules/react-dom/client.js")).toBe(
      true,
    );
    expect(
      shouldIgnorePackagePath(
        "/node_modules/@tanstack/react-router-devtools/dist/index.js",
      ),
    ).toBe(true);
    expect(
      shouldIgnorePackagePath("/node_modules/@esbuild/win32-x64/esbuild.exe"),
    ).toBe(true);
    expect(
      shouldIgnorePackagePath("/node_modules/rollup/dist/bin/rollup"),
    ).toBe(true);
  });
  ...
  it("keeps main-process runtime packages in packaged node_modules", () => {
    ...
    expect(
      shouldIgnorePackagePath("/node_modules/@fastify/cors/index.js"),
    ).toBe(false);
    expect(shouldIgnorePackagePath("/node_modules/@nestjs/core/index.js")).toBe(
      false,
    );
  ...
  it("blocks workspace-only folders from entering app.asar", () => {
    expect(shouldIgnorePackagePath("/.codex/skills/example/SKILL.md")).toBe(
      true,
    );
    expect(shouldIgnorePackagePath("/.agents/skills/example/SKILL.md")).toBe(
      true,
    );
