...
describe('manual update policy', () => {
  ...
  it('normalizes updater.json into release metadata', () => {
    const release = buildGitHubReleaseFromUpdaterJson({
      ...
      url: 'https://github.com/hudiohizari/antigravity-relay/releases/tag/v0.17.0',
    });

    expect(release).toEqual({
      tag_name: 'v0.17.0',
      name: 'v0.17.0',
      html_url: 'https://github.com/hudiohizari/antigravity-relay/releases/tag/v0.17.0',
      draft: false,
      prerelease: false,
    });
  });
  ...
  it('normalizes package.json into release metadata', () => {
    ...
    expect(release?.html_url).toBe(
      'https://github.com/hudiohizari/antigravity-relay/releases/tag/v0.18.0',
    );
  });

  it('normalizes GitHub latest redirect URL into release metadata', () => {
    const release = buildGitHubReleaseFromLatestRedirect(
      'https://github.com/hudiohizari/antigravity-relay/releases/tag/v0.19.0',
    );
