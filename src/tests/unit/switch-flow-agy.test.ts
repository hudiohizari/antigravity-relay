...
describe("executeSwitchFlow for agy CLI", () => {
  ...
  it("syncs telemetry serviceMachineId after SQLite token switching", async () => {
    ...
    expect(
      syncTelemetryServiceMachineIdValue.mock.invocationCallOrder[0],
    ).toBeLessThan(startAntigravity.mock.invocationCallOrder[0]);
  ...
  it("selectively restarts GUI processes only when they were running beforehand", async () => {
    const performSwitch = vi.fn(async () => undefined);
  ...
  it("bypasses startAntigravity and performs silent on-disk injection when target was stopped", async () => {
    const performSwitch = vi.fn(async () => undefined);
    isProcessRunning.mockResolvedValue(false);
