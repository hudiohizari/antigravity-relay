...
export interface SwitchFlowNotifier {
  onSwitchStart?: (callback: () => void) => () => void;
  onSwitchEvent: (callback: (result: SwitchResult) => void) => () => void;
}
...
export class UpstreamBridge {
  ...
  public hookSwitchFlow(switchFlow: SwitchFlowNotifier): () => void {
    const unsubStart = switchFlow.onSwitchStart?.(() => {
