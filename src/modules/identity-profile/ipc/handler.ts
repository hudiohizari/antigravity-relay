...
function getLastKnownGoodStateDbPath(): string {
  return path.join(
    getLastKnownGoodSnapshotDir(),
    LAST_KNOWN_GOOD_STATE_DB_FILE,
  );
}
...
function ensureTelemetryObject(
  storage: Record<string, unknown>,
...
function readStateServiceMachineIdValue(dbPath: string): string | null {
  ...
  try {
    ...
    logger.warn(
      "Failed to read state.serviceMachineId from state.vscdb",
      error,
    );
    return null;
  } finally {
...
function ensureStorageProfileApplied(
...
function ensureStateServiceMachineIdApplied(
...
function pickDeviceProfileTemplate(): DeviceProfileTemplate {
  const templates = DEVICE_PROFILE_TEMPLATES[process.platform] || [
...
function ensureGeneratedProfile(
  profile: DeviceProfile,
...
function ensureStorageStateConsistency(
  ...
  expectedStorage: boolean = true,
): void {
  if (!expectedStorage && !fs.existsSync(storagePath)) {
    return;
  }
  ...
  if (error instanceof Error) {
    if (
      error.message.includes(
        "device_profile_integrity_check_failed_for_storage",
      )
    ) {
      return "verify_storage_failed";
    }
    if (
      error.message.includes(
        "device_profile_integrity_check_failed_for_state_db",
      )
    ) {
      return "verify_state_failed";
    }
...
function markApplyFailure(
  ...
  if (
    deviceHardeningState.consecutiveApplyFailures >=
    DEVICE_HARDENING_SAFE_MODE_THRESHOLD
  ) {
    deviceHardeningState.safeModeUntil =
      now + DEVICE_HARDENING_SAFE_MODE_DURATION_MS;
  }
}
...
function buildApplyError(
  ...
  const wrapped = new Error(
    `device_apply_failed:${reason}:${stage}:${message}`,
  );
  (
    wrapped as Error & {
...
export function getStorageDirectoryPath(
  appTarget?: AntigravityAppTarget,
): string {
  try {
    return path.dirname(getStoragePath(appTarget));
  } catch {
    const defaultStoragePath = getAntigravityStoragePath(appTarget);
    if (defaultStoragePath) {
      return path.dirname(defaultStoragePath);
    }
    throw new Error("storage_json_not_found");
  }
}

function resolveTargetStoragePath(appTarget?: AntigravityAppTarget): {
  storagePath: string;
  hadStorage: boolean;
} {
  const existingStoragePath = getExistingPath(
    getAntigravityStoragePaths(appTarget),
  );
  if (existingStoragePath) {
    return { storagePath: existingStoragePath, hadStorage: true };
  }
  ...
  const storageDir = path.dirname(defaultStoragePath);
  const hasStorageDir = fs.existsSync(storageDir);
  const hasDbFile = getAntigravityDbPaths(appTarget).some((candidate) =>
    fs.existsSync(candidate),
  );
  const hasParentDir = fs.existsSync(path.dirname(storageDir));

  if (!hasStorageDir && !hasDbFile && !hasParentDir) {
    throw new Error("storage_json_not_found");
  }
  ...
  return { storagePath: defaultStoragePath, hadStorage: false };
}
...
export function ensureGlobalOriginalFromCurrentStorage(
  ...
  try {
    ...
    logger.warn(
      "Failed to capture baseline device profile from storage.json",
      error,
    );
  }
}
...
export function syncStateServiceMachineIdValue(
  ...
  const targetDbPath =
    dbPath || getExistingPath(getAntigravityDbPaths(appTarget));
  if (!targetDbPath) {
  ...
  for (let attempt = 1; attempt <= SQLITE_RETRY_COUNT; attempt += 1) {
    ...
    try {
      ...
      if (isSqliteBusyError(error) && attempt < SQLITE_RETRY_COUNT) {
        logger.warn(
          `state.vscdb busy, retrying (${attempt}/${SQLITE_RETRY_COUNT})`,
          error,
        );
        continue;
      }
...
export function applyDeviceProfile(
  ...
  const { storagePath, hadStorage } = resolveTargetStoragePath(appTarget);
  ...
  try {
    ...
    const storage = hadStorage ? parseStorageJson(storagePath) : {};
    ...
    try {
      ...
      logger.warn(
        "Failed to update last known good device snapshot",
        snapshotError,
      );
    }
    ...
    try {
      if (hadStorage && fs.existsSync(backupPath)) {
        fs.copyFileSync(backupPath, storagePath);
      }
      if (!hadStorage) {
        cleanupPathIfExists(storagePath);
      }
      ...
      logger.error(
        "Failed to restore device profile from immediate backup",
        restoreError,
      );
    ...
    if (!restoredFromImmediateBackup) {
      ...
      if (!restoredFromLastKnownGood) {
        logger.error(
          "Failed to restore device profile from last known good snapshot",
        );
