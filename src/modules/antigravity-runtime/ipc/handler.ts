...
const LINUX_GPU_SAFE_LAUNCH_ARGS = [
  "--disable-gpu",
...
function getWindowsTargetImageNames(
  target?: AntigravityAppTarget | null,
): string[] {
...
async function killWindowsTargetImages(
  target?: AntigravityAppTarget | null,
): Promise<boolean> {
  for (const imageName of getWindowsTargetImageNames(target)) {
...
async function getWindowsFindProcessDiagnostic(
  error: unknown,
  ...
  try {
    ...
    return {
      ...
      probableCause:
        "WMI/CIM, permissions, or endpoint policy may be blocking Win32_Process",
    };
  }
}
...
async function reportWindowsFindProcessFailure(error: unknown): Promise<void> {
  const now = Date.now();
  if (
    now - lastWindowsFindProcessDiagnosticAt <
    WINDOWS_FIND_PROCESS_DIAGNOSTIC_INTERVAL_MS
  ) {
    logger.warn(
...
function mapProcessInfoToCandidate(
  processInfo: ProcessInfo,
): AntigravityProcessCandidate {
  const commandLine = processInfo.cmd || processInfo.name || "";
  return {
    name: processInfo.name || "",
    commandLine,
    executablePath:
      processInfo.bin || parseCommandLineArguments(commandLine)[0] || "",
  };
}
...
export async function closeAntigravity(
  ...
  try {
    ...
    logger.info(
      `Found ${targetProcesses.length} remaining ${appName} processes. Killing...`,
    );
...
function shouldUseLinuxGpuSafeLaunchArgs(): boolean {
  ...
  const enableLinuxGpuRaw =
    process.env.ANTIGRAVITY_RELAY_ENABLE_LINUX_GPU?.trim().toLowerCase();
...
async function startAntigravityByExecutable(
  ...
  if (isWsl()) {
    ...
    const quotedArgs = configuredArgs
  ...
  const launchArgs = [
    ...configuredArgs,
    ...(shouldUseLinuxGpuSafeLaunchArgs()
      ? [...LINUX_GPU_SAFE_LAUNCH_ARGS]
      : []),
  ];
...
export async function startAntigravity(
  ...
  if (shouldUseUri) {
    ...
    if (await openUri(uri)) {
      ...
      logger.warn(
        `URI launch did not start ${appName}. Falling back to executable launch.`,
      );
    } else {
