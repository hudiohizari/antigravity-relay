...
export function isElectronUpdaterSupported(
...
function toUpdateNotification(
  ...
  return buildElectronUpdaterNotification({
    ...
    releaseName:
      typeof updateInfo.releaseName === "string"
...
export function registerElectronUpdater(notify: NotifyUpdate): void {
  ...
  appUpdater.logger = {
    info: (message?: unknown) =>
      logger.info(`ElectronUpdater: ${String(message ?? "")}`),
    warn: (message?: unknown) =>
      logger.warn(`ElectronUpdater: ${String(message ?? "")}`),
    error: (message?: unknown) =>
      logger.error(`ElectronUpdater: ${String(message ?? "")}`),
  ...
  if (LOCAL_UPDATE_FEED_URL) {
    logger.warn(
      `ElectronUpdater: using local update feed ${LOCAL_UPDATE_FEED_URL}`,
    );
  }

  if (ALLOW_UNMANAGED_UPDATE_INSTALL) {
    logger.warn(
      "ElectronUpdater: unmanaged install checks are disabled for local verification",
    );
  }
  ...
  appUpdater.on("update-available", (updateInfo) => {
    ...
    notifyUpdate?.(toUpdateNotification("available", updateInfo), {
      force: true,
    });
  });
  ...
  appUpdater.on("download-progress", (progress: ProgressInfo) => {
    logger.info(
      `ElectronUpdater: download progress ${progress.percent.toFixed(2)}%`,
    );
  });
