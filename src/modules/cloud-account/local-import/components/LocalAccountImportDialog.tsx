...
type DialogPhase =
  "idle" | "scanning" | "preview" | "importing" | "result" | "error";
...
const SOURCE_I18N_KEYS = {
  ...
  "antigravity-cli-token": "cloud.localImport.sources.antigravity-cli-token",
} as const;

const VALIDATION_ERROR_I18N_KEYS = {
  "credential-unavailable":
    "cloud.localImport.validationErrors.credential-unavailable",
  "authentication-failed":
    "cloud.localImport.validationErrors.authentication-failed",
...
const IMPORT_ERROR_I18N_KEYS = {
  "credential-unavailable":
    "cloud.localImport.importErrors.credential-unavailable",
...
function isRequestErrorCode(
  value: string,
): value is keyof typeof REQUEST_ERROR_I18N_KEYS {
  ...
  return (
    <div className="space-y-4">
      <div
        ...
        aria-label={t("cloud.localImport.summary")}
      >
        <CountBadge
          label={t("cloud.localImport.accounts")}
        ...
        />
        <CountBadge
          label={t("cloud.localImport.merged")}
          count={preview.merged.length}
        />
      </div>
        ...
        <div className="border-destructive/30 bg-destructive/5 space-y-2 rounded-md border p-3">
          ...
          <ul className="text-muted-foreground space-y-1 text-xs">
            ...
            {preview.discoveryFailures.map((failure, index) => (
              <li
                key={`${failure.source.id}:${failure.source.location ?? ""}:${index}`}
              >
...
export function LocalAccountImportDialog() {
  ...
  const [preview, setPreview] = useState<LocalAccountImportPreview | null>(
    null,
  );
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            {t("cloud.localImport.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          {phase === "preview" && (
            <>
              ...
              >
                {t("cloud.localImport.confirm", {
                  count: preview?.accounts.length ?? 0,
                })}
              </Button>
            </>
          )}
