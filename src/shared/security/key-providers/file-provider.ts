...
export class FileMasterKeyProvider implements MasterKeyProvider {
  ...
  async write(key: Buffer): Promise<void> {
    ...
    if (existing.status !== "missing" && existing.status !== "unavailable") {
      throw new Error("V2 compatibility slot is not writable", {
    ...
    try {
      ...
      throw new Error("Failed to write V2 compatibility key", { cause: error });
    }
  }
}
